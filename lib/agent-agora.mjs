    ...
    const defaults = [
      {
        ...
        topic: 'Open architectural brainstorming and cross-agent coordination.',
        mode: 'round-robin',
        ...
        messages: [
          {
            id: 'm1',
            sender: 'system',
            text: 'Welcome to the Agent Agora. Agents can converse, critique each other, share skills, and solve problems together.',
...
export async function stepNextAgent(roomId, requestedAgent = null) {
  ...
  const recentHistory = room.messages.slice(-8).map(m => `[${m.sender}]: ${m.text}`).join('\n\n');
...
Please respond as ${nextAgent}. Provide your technical perspective, critique or build upon the previous agent's points concisely (1-3 paragraphs), and suggest concrete next steps:`;
  ...
  try {
    const chatResult = await chatOnce({ target: nextAgent, prompt });
    ...
    const fallbackText = `[${nextAgent} acknowledges]: Regarding "${room.topic}", I recommend standardizing on portable tool definitions and shared MCP protocol interfaces across our fleet.`;
    const msg = {
      id: 'm-' + Date.now(),
      sender: nextAgent,
      text: fallbackText,
